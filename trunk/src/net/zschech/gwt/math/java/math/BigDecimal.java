...
//    }
	...
	private static BigDecimal addAndMult10(BigDecimal thisValue,BigDecimal augend, double diffScale) {
        if(diffScale < DOUBLE_TEN_POW.length &&
                Math.max(thisValue.bitLength,augend.bitLength+DOUBLE_TEN_POW_BIT_LENGTH[(int) diffScale])+1<54) {
        ...
        return new BigDecimal(thisValue.getUnscaledValue().add(
                Multiplication.multiplyByTenPow(augend.getUnscaledValue(),(int) diffScale)), thisValue.scale);
    }
    ...
    public BigDecimal subtract(BigDecimal subtrahend) {
        ...
        if (diffScale == 0) {
            ...
            // case s1 > s2 : [ u1 - u2 * 10 ^ (s1 - s2) , s1 ]
            if(diffScale < DOUBLE_TEN_POW.length &&
                    Math.max(this.bitLength,subtrahend.bitLength+DOUBLE_TEN_POW_BIT_LENGTH[(int) diffScale])+1<54) {
            ...
            return new BigDecimal(this.getUnscaledValue().subtract(
                    Multiplication.multiplyByTenPow(subtrahend.getUnscaledValue(),(int) diffScale)), this.scale);
        } else {// case s2 > s1 : [ u1 * 10 ^ (s2 - s1) - u2 , s2 ]
            diffScale = -diffScale;
            if(diffScale < DOUBLE_TEN_POW.length &&
                    Math.max(this.bitLength+DOUBLE_TEN_POW_BIT_LENGTH[(int) diffScale],subtrahend.bitLength)+1<54) {
                return valueOf(this.smallValue*DOUBLE_TEN_POW[(int) diffScale]-subtrahend.smallValue,subtrahend.scale);
            }
            return new BigDecimal(Multiplication.multiplyByTenPow(this.getUnscaledValue(),(int) diffScale)
    ...
    public String toString() {
        ...
        if(bitLength < 32) {
            ...
            toStringImage = Conversion.toDecimalScaledString((long)smallValue,(int)scale);
            return toStringImage;
        }
